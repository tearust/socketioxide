...
pub async fn polling_heartbeat_timeout() {
    ...
    let io = create_server(1234, sender).await;
...
pub async fn ws_heartbeat_timeout() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12344, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn polling_transport_closed() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(1235, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn ws_transport_closed() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12345, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn multiple_http_polling() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(1236, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn polling_packet_parsing() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(1237, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn ws_packet_parsing() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12347, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn client_ns_disconnect() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12348, sender).await;
    let mut rx = attach_handler(&io, 1);
...
pub async fn server_ns_disconnect() {
    ...
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12349, sender).await;
    let io2 = io.clone();
    io.ns("/", move |socket: SocketRef| {
        let id = socket.id;
...
pub async fn server_ws_closing() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12350, sender).await;
...
pub async fn server_http_closing() {
    let (sender, _reciver) = tokio::sync::mpsc::channel(1);
    let io = create_server(12351, sender).await;
    let _rx = attach_handler(&io, 100);
    let mut sids =
